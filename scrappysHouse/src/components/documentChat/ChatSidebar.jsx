...
export default function ChatSidebar({
  ...
  return (
    <aside className="flex w-full flex-col rounded-xl border border-gray-200 bg-white/95 shadow-lg dark:border-gray-700 dark:bg-gray-800/95 lg:h-[calc(100vh-4rem)] lg:w-72 lg:shrink-0 lg:rounded-none lg:border-y-0 lg:border-l-0 lg:shadow-none xl:w-80">
      <div className="border-b border-gray-200 p-2.5 text-left dark:border-gray-700">
        <div className="flex items-center justify-between gap-2">
          <div>
            <p className="text-[11px] font-medium text-purple-600 dark:text-purple-400">
              Document Chat
            </p>
            <h1 className="m-0 text-lg font-semibold text-gray-900 dark:text-white">
              Chats
            </h1>
          </div>
          ...
          <button
            type="button"
            className="rounded-md bg-purple-600 px-2 py-1 text-xs font-medium text-white transition-colors hover:bg-purple-700"
            onClick={() => newChat()}
          >
            New
          </button>
        ...
        <label className="mt-2 block">
          ...
          <input
            ...
            className="h-8 w-full rounded-md border border-gray-300 bg-white px-2 text-xs text-gray-900 outline-none transition-colors placeholder:text-gray-400 focus:border-purple-500 focus:ring-2 focus:ring-purple-200 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 dark:focus:ring-purple-900"
          />
        </label>
          ...
          {multiSelectMode && (
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                className="rounded-md bg-red-600 px-2 py-1 text-xs font-medium text-white transition-colors hover:bg-red-700 disabled:cursor-not-allowed disabled:bg-red-300 dark:disabled:bg-red-900"
                onClick={() => {
      ...
      <div className="flex gap-1.5 overflow-x-auto p-1.5 lg:min-h-0 lg:flex-1 lg:flex-col lg:overflow-y-auto lg:overflow-x-hidden">
        {filteredChats.map((chat) => {
          ...
          return (
            <div
              key={chatId}
              className={`group min-w-56 rounded-md border px-2 py-1.5 text-left transition-colors lg:min-w-0 ${
                isActive
            ...
            >
              <div className="flex items-start gap-1.5">
                {multiSelectMode && (
                  <input
                    ...
                    className="mt-0.5 h-3.5 w-3.5 shrink-0 rounded border-gray-300 text-purple-600 focus:ring-purple-500 dark:border-gray-600 dark:bg-gray-900"
                ...
                >
                  <div className="flex items-start justify-between gap-1.5">
                    <h2 className="m-0 overflow-hidden text-ellipsis whitespace-nowrap text-[13px] font-semibold text-gray-900 dark:text-white">
                      {chatName}
                    </h2>

                    <span className="shrink-0 text-[10px] leading-4 text-gray-500 dark:text-gray-400">
                      {updatedAt}
                    </span>
                  </div>

                  <p className="mt-0.5 overflow-hidden text-ellipsis whitespace-nowrap text-[11px] text-gray-600 dark:text-gray-300">
                    {chatPreview}
                  </p>
                </button>
              </div>
              ...
              <div className="mt-1.5 flex items-center justify-between gap-1.5">
                <div className="inline-flex rounded-full bg-gray-100 px-1.5 py-0.5 text-[10px] font-medium text-gray-600 dark:bg-gray-700 dark:text-gray-300">
                ...
                {!multiSelectMode && (
                  <button
                    type="button"
                    className="rounded-md px-1.5 py-0.5 text-[11px] font-medium text-red-600 opacity-100 transition-colors hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/40 lg:opacity-0 lg:group-hover:opacity-100"
