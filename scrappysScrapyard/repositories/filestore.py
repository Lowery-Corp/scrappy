...
from repositories.minio import get_bucket_structure, upload_file_to_minio
...
async def add_file_to_bucketstore(user_id: str, file_path: str, file: UploadFile, session: AsyncSession) -> dict[str, str | bool]:
    ...
    storage_key: str = f"home/{parsed_file_path}".replace("//", "/")

    if not file.filename:
        return {"message": "Missing filename", "ok": False}

    upload_file_status = await upload_file_to_minio(
        bucket_name=bucket_name,
        file_path=storage_key,
        file_data=await file.read()
    )
    assert upload_file_status["ok"], f"File upload failed: {upload_file_status}"

    if not upload_file_status["ok"]:
